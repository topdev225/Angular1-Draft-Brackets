...
class PickController {
  ...
  public function GetMade(Application $app, Request $request) {
    $draftId = (int)$request->get('draft_id');
    $leagueName = $request->get('league_name');
 

	
    $draft = $app['phpdraft.DraftRepository']->Load($draftId);
	
    return $app->json($app['phpdraft.PickRepository']->LoadMadePicks($draftId, $draft->draft_current_pick,$leagueName));
  }  
	public function SendUserPicks(Application $app, Request $request) {
		$response = $app->json($app['phpdraft.PickRepository']->ZanLoadUserPicks( $request));
