...
//use PhpDraft\Domain\Services\LoginUserService;
...
//Ensure user in contest already 
$contestEntered = function(Request $request, Application $app) {
	
	$contest_id = $request->get('contest_id');
	$bracket_id = (int)$request->get('id');
	
	if($bracket_id == 0) {
		$bracket_id = (int)$request->get('pool_id');
	}
	
	$user = $app['phpdraft.LoginUserService']->GetUserFromHeaderToken($request);
    $user_id = $user->id;

	$viewable = $app['phpdraft.DraftValidator']->isUserInContest($contest_id, $bracket_id,$user_id);
	
	if(!$viewable)  {
		$response = new PhpDraftResponse(false, array());
		$response->errors[] = "You have not entered the contest yet.";
		return $app->json(array($response), $response->responseType());
	}
...
$userJoinedBracket = function(Request $request, Application $app) {
  $bracket_id = (int)$request->get('id');
  die();
  return false;
  $draft = $app['phpdraft.DraftRepository']->Load($draft_id);
...
$actionNotAllowed = function(Request $request, Application $app) {
	  throw new Exception('This action is not allowed.');

	$response = new PhpDraft\Domain\Models\PhpDraftResponse(false, array());
    $response->errors[] = "You do not have permission to do this action.";
...
$commishEditableDraft = function(Request $request, Application $app) {
  ...
  $current_user = $app['phpdraft.LoginUserService']->GetCurrentUser($request);
  ...
  $editable = $app['phpdraft.DraftValidator']->IsDraftEditableForUser($draft, $current_user);
	//CustomAndrae allow commisioners to add themselves
  //if(!$editable) {
