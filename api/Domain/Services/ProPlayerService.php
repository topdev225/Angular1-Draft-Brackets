...
class ProPlayerService {
  ...
  public function SearchPlayers($league, $searchTerm) {
    ...
    try {
      $teams = $this->app['phpdraft.ProPlayerRepository']->SearchPlayers($league, $searchTerm);
      ...
      $split_name_automatically = count($teams) == 0 && strpos($searchTerm, " ") != false;

      if($split_name_automatically) {
        $names = explode(" ", $searchTerm, 2);
        $teams = $this->app['phpdraft.ProPlayerRepository']->SearchPlayersByAssumedName($league, $names[0], $names[1]);
      }
