...
class LoginUserRepository {
  ...
  public function Create(LoginUser $user) {
    ...
    $insert_stmt = $this->app['db']->prepare("INSERT INTO users 
        (id, email, password, salt, name, roles, verificationKey, creationTime, username, dob) 
        VALUES 
        (NULL, ?, ?, ?, ?, ?, ?, UTC_TIMESTAMP(), ?, ?)");
    ...
    $insert_stmt->bindParam(7, $user->username);
    $insert_stmt->bindParam(8, $user->dob);
  ...
  public function NameIsUnique($name, $id = null) {
    if($id == null) {
      $name_stmt = $this->app['db']->prepare("SELECT name FROM users WHERE username LIKE ?");
      $name_stmt->bindParam(1, strtolower($name));
    } else {
      $name_stmt = $this->app['db']->prepare("SELECT name FROM users WHERE username LIKE ? AND id <> ?");
      $name_stmt->bindParam(1, strtolower($name));
      $name_stmt->bindParam(2, $id);
    }

    if(!$name_stmt->execute()) {
      throw new \Exception(sprintf('Username %s is invalid', $name));
    }
