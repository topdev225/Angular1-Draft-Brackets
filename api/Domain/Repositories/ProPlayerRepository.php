...
class ProPlayerRepository {
  ...
  public function SearchPlayers($league, $searchTerm) {
	//$allTeams = $this->app['mongodb']->draftbrackets->teamcollection->find("name"=> "NBA");
	//since will be getting the league as an id need to convert to name 
	$filter = [
				'league' => $league,
				'name' =>  array('$regex' => ".*$searchTerm.*", '$options' =>'$i')
				 //db.teamcollection.find({'name': {'$regex': 'Golden State Warriors'}});

	
				]; 
	$options = array(); 
  
	$client = $this->app['mongodb'];
	
	$rows = $client->draftbrackets->teamcollection->find($filter,$options);
	//$collection = new Collection($manager, 'draftbrackets', 'teamcollection');
	
	//$jokesRow = iterator_to_array($rows);
	
	$arrayInside = array();
	foreach ($rows as $row) {
		$arrayInside[] = $row;
	}
	$jsonInside = json_encode($arrayInside);
	/*
    //Limit the amount of searches to just 15 to cut down on unnecessary network traffic
