...
class DraftValidator {
  ...
  public function __construct(Application $app) {
	...
	public function isUserInContest($contest_id, $pool_id,$user_id) {
		 $exists = $this->app['db']->prepare("SELECT COUNT(*) AS 'Entered' FROM managers WHERE contest_id = ? AND user_id = ? AND draft_id = ?  ");
		$exists->bindParam(1, $contest_id);
		$exists->bindParam(2, $user_id);
		$exists->bindParam(3, $pool_id);
		$exists->execute();
		$count = $exists->fetchColumn(0);
		
		if ($count == 0) {
		  throw new \Exception("You have to enter the contest first to play.");
		}
		return $count;
  ...
  public function IsDraftComplete(Draft $draft) {
    ...
    //This checks if it's the manager's turn so can all them to see the edit picks menu
  public function checkIfManagerTurn($draft, $request) {
	//Can only execute everything if
		//First get the current pick if inside a draft
			//var_dump($request);
			//var_dump($draft);
			$whosTurn = "";
			
			if($draft->draft_status !== 'undrafted') {
				$currentPick = $this->app['phpdraft.PickRepository']->GetCurrentPick($draft);
				$whosTurn = $currentPick->user_id;
			}
			
			//Then grab the user id from the manager id so can use for the logged in user and see if it is his/her's turn
			$current_user = $this->app['phpdraft.LoginUserService']->GetUserFromHeaderToken($request);

			if($whosTurn == $current_user->id) {
				return true;
			}else {
				return false;
			}
